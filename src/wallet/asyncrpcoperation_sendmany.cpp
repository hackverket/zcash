...
// WE SHOULD COMMENTIT OUT
        ...
        // fix: send right here if z_outputs==0
    }
    ...
    if (z_outputs_total>0) {
        ...
        while (zOutputsDeque.size() > 0) {
            ...
            if (vInputNotes.size() > 0) {
                ...
                {
                    // TODO MAYBE: inputAnchor, set it to best anchor if there is no change?
                    LOCK(cs_main);
                    pwalletMain->GetNoteWitnesses(vOutPoints, vInputWitnesses, inputAnchor);
                    // Reminder: Another way to obtain witnesses is to scan the blockchain (it's slow)
...
// generate 1 (make sure in blockchain))
// getnewaddress
// z_sendmany zaddr newtaddr (any amoung e.g. 1.0)
...
#if 0
            std::cout << "IF SWITCH IS 1: Let's continue making joinsplit with output from WitnessNoteCommitment()" << std::endl;
#else
            std::cout << "IF SWITCH IS 0: GAME OVER: change this switch to make joinsplit with output from WitnessNoteCommitment()" << std::endl;
